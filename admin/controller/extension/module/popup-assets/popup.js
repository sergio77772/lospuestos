(function($) {
    ...
    let $deactivateForm  = jQuery('#popup-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('popup-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'popup-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('popup-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_popup.php', jQuery.param(data)).then(function(result) {
