...
class ControllerExtensionTotalXfeepro extends Controller {
    ...
    public function index() {
       ...
       if (isset($this->error['warning'])) {
        ...
        $this->latencyImport();
        $data['method_data'] = $this->model_extension_total_xfeepro->getData();
    ...
    public function quick_save() {
       ...
       if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
           ...
           if(isset($this->request->post[$this->meta['name']]) && isset($this->request->post['tab_id'])) {
               ...
               $this->model_extension_total_xfeepro->addData($save);
               $json['success'] = true;
           } else {
    ...
    public function delete() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            if($this->request->post['tab_id']) {
                $this->model_extension_total_xfeepro->deleteData($this->request->post['tab_id']);
                $json['success'] = true;
            }
            else {
    ...
    public function export(){
        ...
        $this->load->model($this->ext_path);
        $export['method_data'] = $this->model_extension_total_xfeepro->getData();
        ...
        header("Content-Disposition: attachment; filename=xfeepro.txt");
        echo $out;
        exit;
    } 
    ...
    public function exportMethodRangeRows($tab_id) {
        $this->load->model($this->ext_path);
        $method_row = $this->model_extension_total_xfeepro->getDataByTabId($tab_id);
        ...
        $heading = array('{start}','{end}','{cost}','{block}','{partial}');
    ...
    public function import() {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && is_uploaded_file($this->request->files['file_import']['tmp_name']) && file_exists($this->request->files['file_import']['tmp_name'])) {
            ...
            if ($import_data) {
                $import_data = $this->model_extension_total_xfeepro->getUnCompressedData($import_data);
                if (is_array($import_data) && (isset($import_data['method_data']) || isset($import_data['xfeepro']))) {
                    $method_data = array();
                    $xfeepro = '';
                    ...
                    if (isset($import_data['xfeepro'])) {
                        $xfeepro = $import_data['xfeepro'];
                        unset($import_data['xfeepro']);
                    }
                    ...
                    foreach($method_data as $single) {
                        ...
                        $this->model_extension_total_xfeepro->addData($single);
                    }
                    if ($xfeepro) {
                        $this->latencyImport($xfeepro);
                    }
                    $success = true;
                }
            }
    ...
    public function latencyImport($xfeepro = '') {
        if (!$xfeepro) {
            $xfeepro = $this->config->get('xfeepro');
        }
        if (!$xfeepro) return false;
        $this->load->model($this->ext_path);
        $xfeepro = unserialize(base64_decode($xfeepro)); 
        
        if (!is_array($xfeepro)) $xfeepro = array();
        $methods = array();
        foreach($xfeepro as $key => $value) {
        ...
        $this->db->query("DELETE FROM `" . DB_PREFIX . "setting` WHERE `key` = 'xfeepro'");
        $this->db->query("UPDATE `" . DB_PREFIX . "order_total` SET `code` = 'xfeepro' WHERE `code` like 'xfeepro.xfeepro%'");
        return true;
    }
    ...
    private function getFormData($data, $new_tab = false) {
        ...
        /* Set base name for form inputs */
        $this->ocm->form->setBasename('xfeepro');
    ...
    private function getGroups($data) {
        ...
        for ($i=1; $i <= $data['groups_count']; $i++) {
            ...
            $current_method_name =  isset($data[$prefix . 'xfeepro_group_name'][$i]) ? $data[$prefix . 'xfeepro_group_name'][$i]:'';

            $return .='<tr>
                <td class="text-left">Group'.$i.'</td>
                <td class="text-left">
                <select class="xfeepro_group'.$i.'" name="' . $prefix . 'xfeepro_group['.$i.']">';

            foreach ($data['group_modes'] as $type => $name) {
                $selected = (isset($data[$prefix . 'xfeepro_group'][$i]) && $data[$prefix . 'xfeepro_group'][$i]==$type) ? 'selected':'';
                $current_method_mode = (isset($data[$prefix . 'xfeepro_group'][$i]) && $data[$prefix . 'xfeepro_group'][$i]==$type)? $type: $current_method_mode;
    ...
    public function fetchZone() {
        $country = isset($this->request->post['xfeepro']) && isset($this->request->post['xfeepro']['country']) && $this->request->post['xfeepro']['country'] ? $this->request->post['xfeepro']['country'] : array();
    ...
    public function getProducts() {
        ...
        $json = $this->model_extension_total_xfeepro->getProducts($filter_data);
        $this->response->addHeader('Content-Type: application/json');
        $this->response->setOutput(json_encode($json)); 
    }
    ...
    public function install() {
        $this->load->model($this->ext_path);
        $this->model_extension_total_xfeepro->addDBTables();
        $this->ocm->util->addEvents($this->events);
    }

    public function uninstall() {
        $this->load->model($this->ext_path);
        $this->model_extension_total_xfeepro->removeDBTables();
